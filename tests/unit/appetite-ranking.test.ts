...
function packet(id: string, tier: DecisionPacket["tier"], targetAlignment = 0.5): DecisionPacket {
  return {
    submission: { id, accountName: id, primaryState: "OH", premium: 80_000, totalInsuredValue: 70_000_000, receivedAt: "2026-09-19T12:00:00.000Z" },
...
describe("decision ranking", () => {
  it("orders tiers before score and uses the documented tier sequence", () => {
    const ranked = rankDecisionPackets([
      ...
      packet("now", "review_now", 0),
    ]);
    expect(ranked.map((item) => item.submission.id)).toEqual(["now", "standard", "info", "manual", "decline"]);
  });
